...
_Senales encontradas: T1(cross-indication)=25 | T2(subpoblacion)=4 | T3(outcome inesperado)=28 | T4(hidden gems)=110 | T6(temporal)=6 | T7(contradicciones)=12 | T8(kg-convergence)=12 | T9(pleiotropia)=48_
...
El scan revela un campo en transición: mientras que las señales de GLP-1, Metformina y microbiota (T1, T8) actúan como anclas de consenso ya maduras, emerge un desplazamiento hacia la optimización de subpoblaciones y la medicina de precisión. El patrón global indica que la eficacia de las intervenciones está fuertemente estratificada por el IMC y la etnia (T2b), sugiriendo que el PCOS no es una entidad monolítica sino un conjunto de fenotipos metabólicos. La señal más disruptiva y genuinamente nueva es la de los inhibidores de SGLT2 (T1), que presenta una consistencia del 88% en estudios con resultados, superando en robustez estadística a fármacos más estudiados como la melatonina o el exenatide.

## Señales prioritarias para investigar

1. **Inhibidores de SGLT2**: Es la señal más potente y menos saturada (23 papers vs >80 de liraglutida). A diferencia de la metformina, que actúa en la producción hepática de glucosa, los SGLT2 inducen glucosuria, reduciendo la carga glucémica sistémica y el estrés oxidativo sin pasar necesariamente por la sensibilización insulínica clásica. Aparece en T1 con un score de 7.442. **Validación**: RCT doble ciego comparando SGLT2 vs Metformina en pacientes con PCOS y obesidad, midiendo específicamente la tasa de ovulación y niveles de andrógenos libres.

2. **Terapia de Ciclo Artificial / HRT en Transferencia de Embriones (T4)**: Aunque es una señal de un solo estudio, el N es masivo (>8,000 participantes), lo que le otorga una potencia estadística superior a la mayoría de los RCTs de suplementos. Sugiere que la manipulación del entorno endometrial mediante ciclos artificiales optimiza el outcome reproductivo en PCOS más que los ciclos naturales modificados. **Validación**: Estudio multicéntrico comparativo de tasas de implantación y complicaciones placentarias entre ciclos naturales vs. artificiales en pacientes con PCOS.

3. **L-Carnitina**: Presenta una consistencia del 80% y es "parcialmente estudiada" (24 papers), situándose en el punto dulce entre la novedad y la evidencia. Su mecanismo plausible es la optimización de la $\beta$-oxidación mitocondrial en las células de la teca, reduciendo la disponibilidad de acetil-CoA para la síntesis de andrógenos. **Validación**: Ensayo clínico midiendo la relación andrógenos/estrógenos en folículo antral tras suplementación con L-carnitina.

## Hipótesis mecanísticas emergentes

1. **Sinergia Glucosa-Andrógenos vía SGLT2**: Si la reducción de la glucosa plasmática mediante la excreción renal (SGLT2) disminuye la señalización de insulina en el ovario de forma más abrupta que los sensibilizadores, entonces observaremos una caída de andrógenos más rápida en pacientes con IMC >30 que en pacientes magras. Esta hipótesis combina la señal de SGLT2 (T1) con la alta respuesta de la subpoblación obesa (T2b). **Test**: Medición de testosterona libre pre y post tratamiento con SGLT2 estratificado por IMC.

2. **Efecto Etnia-Específico en la Respuesta a Suplementos**: Si la respuesta superior de la población iraní (T2b, 87% favorable) se debe a una variante genética en el transporte de inositoles o receptores de andrógenos, entonces la combinación de Magnesio/Melatonina será significativamente más efectiva en este grupo que en el grupo chino. Combina T1 (Melatonina) con T2b (Etnia). **Test**: Estudio de farmacogenómica comparativo entre cohortes iraníes y chinas.

## Señales que son probablemente ruido

- **Melatonina y Liraglutida**: Ruido por saturación. Son señales "favorables" pero ya forman parte del consenso o están en meta-análisis; no aportan novedad científica.
- **Cangfu Daotan / Dingkun Pill / Zishen Yutai**: Ruido por sesgo de publicación regional y falta de replicación. Son señales T4 (un solo estudio) con mecanismos no plausibles biológicamente en la literatura occidental y alta probabilidad de sesgo en el diseño del estudio.
- **Espironolactona**: La señal T6 (caída post-2021) sugiere que la evidencia inicial estaba inflada y que los estudios más rigurosos y recientes están refutando su eficacia superior, convirtiéndola en una señal de "no-replicación".

---

## 🔍 Verificación de Novedad — Búsqueda en PubMed

> Esta sección muestra los resultados de la búsqueda en PubMed que se
> pasaron al LLM para que evaluara la novedad de cada señal.
> Si hay >80 papers en PubMed → señal establecida, no nueva.
> Si hay 0 papers → hipótesis genuinamente emergente.

```
=== VERIFICACIÓN NOVEDAD T8 (hipótesis A→B→C) ===
  Pregunta: ¿existe ya algún estudio que mida la conexión DIRECTA A→C?
  Si SÍ → hipótesis conocida. Si NO → genuinamente emergente.

  [metformin --[improves]--> insulin resistance --[associated_with]--> pcos]
    PubMed PCOS: 🔴 2008 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → Metformin use in women with polycystic ovary syndrome (PCOS): Opportunities, benefits, and clinical challenges. (2025)
      → Comparison of two insulin sensitizers, metformin and myo-inositol, in women with polycystic ovary syndrome (PCOS). (2017)
    DB interna: 2 arco(s) directo(s) A→C encontrado(s)
      KG: metformin --[tested_in]--> pcos [10.1186/s12887-020-1960-2]
      KG: metformin --[tested_in]--> pcos [10.2147/ppa.s244273]

  [insulin resistance --[associated_with]--> obesity --[associated_with]--> pcos]
    PubMed PCOS: 🔴 6128 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → The latest reports and treatment methods on polycystic ovary syndrome. (2024)
      → Insulin resistance in polycystic ovary syndrome across various tissues: an updated review of pathogenesis, evaluation, a (2023)
    DB interna: 3 arco(s) directo(s) A→C encontrado(s)
      KG: insulin resistance --[associated_with]--> pcos [10.1186/s13048-025-01621-6]
      KG: insulin resistance --[activates]--> pcos [10.3390/jcm12031221]

  [gut microbiota --[associated_with]--> insulin resistance --[associated_with]--> pcos]
    PubMed PCOS: 🔴 280 papers — conexión A→C YA ESTUDIADA en PCOS
      → Gut microbiota-bile acid-interleukin-22 axis orchestrates polycystic ovary syndrome. (2019)
      → Polycystic Ovary Syndrome: Etiology, Current Management, and Future Therapeutics. (2023)
      → Gut Microbiota and Polycystic Ovary Syndrome (PCOS): Understanding the Pathogenesis and the Role of Probiotics as a Ther (2024)
    DB interna: sin arcos A→C (32% cobertura)

  [insulin resistance --[associated_with]--> hyperandrogenism --[associated_with]--> pcos]
    PubMed PCOS: 🔴 6128 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → The latest reports and treatment methods on polycystic ovary syndrome. (2024)
      → Insulin resistance in polycystic ovary syndrome across various tissues: an updated review of pathogenesis, evaluation, a (2023)
  ...
  [obesity --[increases]--> oxidative stress --[associated_with]--> pcos]
    PubMed PCOS: 🔴 4609 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → Polycystic ovary syndrome: definition, aetiology, diagnosis and treatment. (2018)
      → Pathophysiology and Nutritional Approaches in Polycystic Ovary Syndrome (PCOS): A Comprehensive Review. (2023)
    DB interna: 3 arco(s) directo(s) A→C encontrado(s)
      KG: obesity --[associated_with]--> pcos [10.1186/s13048-021-00879-w]
      KG: obesity --[associated_with]--> pcos [10.1186/s12916-022-02238-y]

  [metformin --[improves]--> insulin resistance --[associated_with]--> obesity]
    PubMed PCOS: 🔴 643 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → Polycystic ovary syndrome. (2022)
      → Obesity, Polycystic Ovary Syndrome, and Infertility: A New Avenue for GLP-1 Receptor Agonists. (2020)
    DB interna: sin arcos A→C (32% cobertura)

  [metformin --[improves]--> insulin resistance --[associated_with]--> hyperandrogenism]
    PubMed PCOS: 🔴 480 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic ovary syndrome. (2022)
      → Polycystic ovary syndrome (PCOS), an inflammatory, systemic, lifestyle endocrinopathy. (2018)
      → Polycystic ovary syndrome: a "risk-enhancing" factor for cardiovascular disease. (2022)
    DB interna: sin arcos A→C (32% cobertura)

  [insulin resistance --[associated_with]--> obesity --[increases]--> oxidative stress]
    PubMed PCOS: 🔴 447 papers — conexión A→C YA ESTUDIADA en PCOS
      → Polycystic Ovary Syndrome: A Comprehensive Review of Pathogenesis, Management, and Drug Repurposing. (2022)
      → Polycystic Ovary Syndrome: Etiology, Current Management, and Future Therapeutics. (2023)
      → A brief insight into the etiology, genetics, and immunology of polycystic ovarian syndrome (PCOS). (2022)
    DB interna: 1 arco(s) directo(s) A→C encontrado(s)
      KG: insulin resistance --[increases]--> oxidative stress [10.1186/s12958-024-01337-0]

  [obesity --[increases]--> oxidative stress --[reduces]--> shbg]
    PubMed PCOS: 🔴 348 papers — conexión A→C YA ESTUDIADA en PCOS
      → [META-ANÁLISIS] Polycystic ovary syndrome and recurrent pregnancy loss, a review of literature. (2023)
      → [META-ANÁLISIS] Effectiveness of Probiotics, Prebiotics, and Synbiotics in Managing Insulin Resistance and Hormonal Imbalance in Women w (2024)
      → Obesity contributes to telomere shortening in polycystic ovary syndrome. (2024)
    DB interna: 1 arco(s) directo(s) A→C encontrado(s)
      KG: obesity --[reduces]--> shbg [10.7570/jomes24035]

  [gut microbiota --[associated_with]--> insulin resistance --[associated_with]--> obesity]
    PubMed PCOS: 🔴 93 papers — conexión A→C YA ESTUDIADA en PCOS
      → Gut microbiota-bile acid-interleukin-22 axis orchestrates polycystic ovary syndrome. (2019)
      → Polycystic Ovary Syndrome: Etiology, Current Management, and Future Therapeutics. (2023)
      → Gut microbiota: a hidden player in polycystic ovary syndrome. (2025)
    DB interna: sin arcos A→C (32% cobertura)

=== NIVEL DE ESTABLECIMIENTO T1 (cuánto ya se sabe de cada señal) ===
  PubMed: '{drug} polycystic ovary syndrome' — si >30 papers, probable consenso.

  [sglt2 inhibitors]: ⚠️  PARCIALMENTE ESTUDIADO — 23 papers en PCOS
  [melatonin]: 🔴 MUY ESTUDIADO — 114 papers en PCOS → probablemente no es señal nueva
  [exenatide]: 🟡 BASTANTE ESTUDIADO — 45 papers en PCOS (revisar si hay meta-análisis)
  ...
  [intermittent fasting]: ⚠️  PARCIALMENTE ESTUDIADO — 19 papers en PCOS
  [glp-1 receptor agonists]: 🟡 BASTANTE ESTUDIADO — 63 papers en PCOS (revisar si hay meta-análisis)
  [liraglutide]: 🔴 MUY ESTUDIADO — 82 papers en PCOS → probablemente no es señal nueva
  [cognitive behavioral therapy]: ⚠️  PARCIALMENTE ESTUDIADO — 26 papers en PCOS
    → [META-ANÁLISIS] The effects of cognitive behavioral therapy in women with polycystic ovary syndrome: A meta-analysis. (2022)
  [l-carnitine]: ⚠️  PARCIALMENTE ESTUDIADO — 24 papers en PCOS
    → [META-ANÁLISIS] Effects of L-carnitine supplementation for women with polycystic ovary syndrome: a systematic review and meta-analysis. (2022)
...
### [T1] melatonin
- Estudios: 13 | Favorables: 5 | Mixed: 1 | Consistencia: 83% | Rareza: 0.26 | **Score: 5.233**

| Ano | ID | Tipo | Efecto | Outcomes |
|---|---|---|---|---|
| nan | NCT01540747 | None | None | n/a |
| nan | NCT02663570 | None | None | n/a |
| 2025.0 | 10.1093/jsxmed/qdaf028 | None | favorable | female sexual function index, hospital anxiety and depression scale, modified pcos health-related qol questionnaire |
| 2025.0 | 10.1007/s00018-025-05609-9 | None | favorable | ovarian mitochondrial dysfunction, clock gene expression, pyroptosis |
| nan | NCT07307677 | None | None | n/a |
...
| 2021.0 | 10.1186/s13048-021-00912-y | None | unclear | mitochondrial membrane potential, sirt1 expression, p-akt level |
| 2021.0 | 10.1530/rep-20-0643 | None | unclear | ovarian function, autophagy, serum testosterone |
